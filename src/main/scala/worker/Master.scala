...
class Master(workTimeout: FiniteDuration) extends PersistentActor with Timers with ActorLogging {
  ...
  // persistentActor with a globally unique name should be a global singleton
  ...
  // every time this method is called, a 'Deadline' object will be created = now + 'consideredWorkerDeadAfter'
  def newStaleWorkerDeadline(): Deadline = considerWorkerDeadAfter.fromNow

  // different from singleTime, this one stays as long as the Master stays and periodically send out CleanupTick
  ...
  // invoked when this Master starts up
  ...
  override def receiveCommand: Receive = {
    // update 'workers' map and workState periodically upon receiving msg 'RegisterWorker' from a worker
    case MasterWorkerProtocol.RegisterWorker(workerId) =>
      if (workers.contains(workerId)) {
        // as long as the worker is alive, keep updating 'staleWorkerDeadline'
    ...
    // #graceful-remove
    // worker upon being stopped, send msg 'DeRegisterWorker' to Master to gracefully de-register itself.
    case MasterWorkerProtocol.DeRegisterWorker(workerId) =>
      workers.get(workerId) match {
        case Some(WorkerState(_, Busy(workId, _), _)) =>
          ...
          // the event 'WorkerFailed(workId)' will persist on disk as a log entry before the event handling is trigger
          // during recovery phase, 'this' Master will switch to receiveRecovery state and
          // all the persisted event log will be replayed and all the event handling will go through once again to reach where it left off.
        ...
        workers.get(workerId) match {
          case Some(workerState @ WorkerState(_, Idle, _)) =>
            ...
            persist(WorkStarted(work.workId)) { event =>
              ...
              workers += (workerId -> workerState.copy(status = Busy(work.workId, Deadline.now + workTimeout), staleWorkerDeadline = newStaleWorkerDeadline()))
              sender() ! work
            }
          case _ =>
        }
