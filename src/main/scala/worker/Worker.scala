...
class Worker(masterProxy: ActorRef)
  ...
  // schedule msg 'RegisterWork' to be sent periodically every 'registerInterval' seconds
  ...
  def working: Receive = {
    case WorkExecutor.WorkComplete(result) =>
      ...
      // schedule msg ReceiveTimeout sent to itself after 5s
  ...
  def waitForWorkIsDoneAck(result: Any): Receive = {
    // Ack from master indicating 'work is done'
    case Ack(id) if id == workId =>
      masterProxy ! WorkerRequestsWork(workerId)
      context.setReceiveTimeout(Duration.Undefined)
      context.become(idle)

    // under this 'waitForWorkIsDoneAck' state, the worker keeps loop until Ack arrives
    case ReceiveTimeout =>
  ...
  override def supervisorStrategy = OneForOneStrategy() {
    // init failure => stop => postStop() => de-register itself
    case _: ActorInitializationException => Stop
    // all other exceptions, restart the replace 'this' worker actor with a new one
    case _: Exception =>
